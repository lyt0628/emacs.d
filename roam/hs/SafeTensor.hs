...
-- :header-args: :tangle hs/SafeTensor.hs :comments both
-- :END:
...
class TensorAdd t1 t2 where
  type AddResult t1 t2 :: Type
  add :: Num a => t1 a -> t2 a -> AddResult t1 t2

instance Num a => TensorAdd (Tensor dims a) (Tensor dims a) where
  type AddResult (Tensor dims a) (Tensor dims a) = (Tensor dims a)
